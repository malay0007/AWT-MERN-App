...
export default function Home() {
  ...
  useEffect(() => {
    axios.get('/api/scores/me').then(({ data }) => setMyScores(data.data)).catch(() => {});
  }, []);

  const best   = myScores.length ? Math.max(...myScores.map(s => s.score)) : null;
    ...
    <div className="page-wrap fade-up">
      ...
      <div className="card" style={{ marginBottom: '1.5rem', textAlign: 'center', padding: '3rem 2rem' }}>
        ...
        <p style={{ color: 'var(--muted)', fontSize: '15px', margin: '0.75rem 0 2rem', maxWidth: '400px', marginLeft: 'auto', marginRight: 'auto' }}>
          Welcome, <strong style={{ color: 'var(--text)' }}>{user?.name}</strong>! Test your knowledge on
          React, Node.js, Express, MongoDB, JWT Security &amp; GitHub.
        </p>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px', marginBottom: '1.5rem' }}>
        {[
          { label: 'Attempts',   val: myScores.length },
          { label: 'Best score', val: best !== null ? best + '%' : '—' },
          { label: 'Last score', val: latest ? latest.score + '%' : '—' },
        ].map(({ label, val }) => (
          <div key={label} style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: '14px', padding: '1.25rem', textAlign: 'center' }}>
            <div style={{ fontFamily: 'var(--font-head)', fontSize: '28px', fontWeight: 800, color: val === '—' ? 'var(--muted)' : 'var(--purple)' }}>{val}</div>
      ...
      {myScores.length > 0 && (
        <div className="card">
          <h3 style={{ fontSize: '15px', fontWeight: 600, marginBottom: '1rem' }}>Recent attempts</h3>
          <div className="table-wrap">
            <table>
              <thead>
                <tr><th>#</th><th>Date</th><th>Score</th><th>Correct</th><th>Avg time</th></tr>
              </thead>
              <tbody>
                {myScores.slice(0, 5).map((s, i) => (
                  <tr key={s._id}>
                    <td style={{ color: 'var(--muted)' }}>{i + 1}</td>
                    <td>{new Date(s.createdAt).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}</td>
                    <td>
      ...
      <div className="card" style={{ marginTop: '1.5rem' }}>
        <h3 style={{ fontSize: '15px', fontWeight: 600, marginBottom: '1rem' }}>Topics covered</h3>
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          {['React', 'Node.js', 'Express', 'MongoDB', 'JWT Security', 'bcrypt', 'REST APIs', 'GitHub'].map(t => (
            <span key={t} className="badge badge-purple" style={{ fontSize: '12px', padding: '5px 12px' }}>{t}</span>
          ))}
        </div>
      </div>
    </div>
